  ...
  while (err2>=0 ) {
   ...
   if(oldyr>1900){
    for(i=0;i<10;i++){
      ...
      /*OUTPUT */
      if (oldmon<11){
        char min_str[32];  // Assuming a maximum string length of 32 characters.
        char max_str[32];  // Adjust the size as needed.

        // Convert min and max to strings.
        snprintf(min_str, sizeof(min_str), "%.1f", min);
        snprintf(max_str, sizeof(max_str), "%.1f", max);

        // Check and remove '$' from the strings if present.
        char* dollar_sign = strchr(min_str, '$');
        if (dollar_sign) {
            *dollar_sign = '0';
        }

        dollar_sign = strchr(max_str, '$');
        if (dollar_sign) {
            *dollar_sign = '0';
        }

        min = atof(min_str);  // Convert the cleaned strings back to float.
        max = atof(max_str);
