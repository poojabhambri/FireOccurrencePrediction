...
ALBERTA_ICON_FILE = os.path.abspath(os.path.join(os.path.dirname(os.path.dirname(__file__)), 'FireOccurrencePrediction\\FireOccurrencePrediction\\resource_files\\alberta_wildfire_icon.ico'))
...
def run_human_fop_model( __config,folder_path):
    if st.session_state.selected_date:
        ...
        __hmn_fire_confidence_interval = '95'
...
def show_outputs(folder_path):
    ...
    if png_files:
        ...
        st.write("No PNG images found in the folder.")
...
def run_lightning_model(__config,folder_path):
    if st.session_state.selected_date:
        __date_to_predict_for_str = st.session_state.selected_date.strftime("%Y-%m-%d")
        __date_to_predict_for = datetime.datetime.strptime(__date_to_predict_for_str, "%Y-%m-%d")
        int_ltg_fire_holdover_lookback_time = 12
        __ltg_fire_confidence_interval = 95
        __display_historical_fires_on_maps = 'True'
...
if __name__ == "__main__":
    ...
    __config.read('FireOccurrencePrediction\\config.ini')
    clearSystemStateDB()
    make_menu()
    ...
    if model:
        if (
            ...
            st.session_state.history is not None and  st.session_state.selected_date is not None
        ):
            if model == "Human":
