...
def make_menu():
    ...
    end_date = st.sidebar.date_input("Select an end date for the date range", datetime.date.today())
    if end_date >= selected_date:
        st.session_state.end_date = end_date
        st.success("Date entered successfully")
    else:
    ...
    try:
        if st.session_state.selected_date and not st.session_state.show_outputs:
            ...
            with st.spinner("Running Human FOP Model..."):
                ...
                pool = multiprocessing.Pool(processes=1)
                if st.session_state.end_date and st.button("Click Here To Use Date Range"):
